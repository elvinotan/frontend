<template>
  <EPageLoader id="fetcher" ref="fetcher" label="Load Data Family" :fetcher="fetcher">
...
export default {
  ...
  methods: {
    async fetcher() {
      ...
      if (param.from !== 'familyList') return false // sumber bukan berasal dari familyList, return error
      if (!param.familyId) return true // ini artinya ari list tp gax ada familyId, artinya newDataFamilyId

      const { result, error } = await this.$rest.get(`/family/fetch/${param.familyId}`)
      if (result) {
        this.model = this.deconstructFamily(result)
        return true // success fetching dta
      }
      if (error) return false // error saat fetch data
    },
    fillNewFamilyData() {
      this.model.id = null
